...
if [ -z "$PRIVATE_KEY" ]; then
	PRIVATE_KEY=$(docker compose logs | grep -i "key" | grep -oE "0x[0-9a-fA-F]{64}" | head -1)
fi
...
NODE_ADDRESS=$(docker compose logs | grep -E "Node address|Ethereum address" | grep -oE "0x[0-9a-fA-F]{40}" | head -1)
