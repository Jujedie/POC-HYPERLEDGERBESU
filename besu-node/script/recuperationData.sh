...
echo "docker logs $3"
while ! docker logs $3 | grep -q "Enode URL"; do sleep 1; done

ENODE=$(docker logs $3 | grep "Enode URL" | cut -d '|' -f 5 | cut -d ' ' -f 4 )
...
NODE_ADDRESS=$(docker logs $3 | grep -E "Node address|Ethereum address" | grep -oE "0x[0-9a-fA-F]{40}" | head -1)
